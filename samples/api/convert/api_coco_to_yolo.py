...
def demo_segmentation_mode(coco_json, output_dir):
    ...
    print(f"\nCode:")
    print(f"  import dataflow")
    print(f"  result = dataflow.coco_to_yolo('{coco_json}', '{output_dir}', segmentation=True)")

    print(f"\nExecuting...")
    try:
        result = dataflow.coco_to_yolo(coco_json, output_dir, segmentation=True)
...
def demo_error_handling():
    ...
    try:
        ...
        try:
            result = dataflow.coco_to_yolo(test_paths["nonexistent_path"],
        ...
        try:
            result = dataflow.coco_to_yolo(test_paths["temp_file_path"],
        ...
        try:
            ...
            try:
                result = dataflow.coco_to_yolo(bad_json, os.path.join(test_paths["temp_dir"], "output"))
                print(f"   ❌ Should have raised an error")
...
def main():
    ...
    try:
        ...
        print(f"   3. Use segmentation=True for polygon annotations")
