...
27. **Coordinate clamping**: All handlers clamp out-of-bounds coordinates before validation:
   - **YOLO**: Bbox edges and polygon points are clamped to `[0, 1]` via `_clamp_normalized_bbox()` / `_clamp_normalized_points()`. Change detection threshold is `5e-7` (half of YOLO `.6f` output precision) — sub-threshold changes are silent to suppress string↔float round-trip noise.
