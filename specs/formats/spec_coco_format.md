...
- `annotations` array — each entry includes `score` (see §10.2)
...
**Loading strategy**: Variant B must be loaded via `pycocotools.COCO.loadRes()`, which copies `images` and `categories` from the GT and indexes the annotations list. The `COCO()` constructor rejects this format (it requires a dict). See `spec_evaluate.md` §10.1 for the loading pipeline.
